...
function setup(isEditing: boolean) {
...
async function renderItem(isEditing: boolean) {
  const { gridItem } = setup(isEditing);
  render(<gridItem.Component model={gridItem} />);
  // VizPanel loads its plugin asynchronously, flush that update inside act()
  // so it doesn't trigger a console warning.
  await act(async () => {
...
describe('AutoGridItemRenderer', () => {
  it('shows a disabled resize hint while editing', async () => {
    await renderItem(true);

    expect(screen.getByLabelText(RESIZE_DISABLED_LABEL)).toBeInTheDocument();
  });

  it('does not show the resize hint when not editing', async () => {
